...
token = "1357908894:AAFxR0jZzEYMpC9G_g5xlswU0OPPes7mFq4"
bot = telebot.TeleBot(token)


# Заготовка для прогнозов
...
def get_text_message(message):
    if message.text == "Привет":
        ...
        key_kozerog = types.InlineKeyboardButton(text='Козерог', callback_data='capricon')
        keyboard.add(key_kozerog)
        key_vodoley = types.InlineKeyboardButton(text='Водолей', callback_data='aquaries')
...
def callback_worker(call):
    ...
    if call.data == "sagittarius":
        msg = random.choice(first) + ' ' + random.choice(second) + ' ' + random.choice(second_add) + ' ' + random.choice(third)
        bot.send_message(call.message.chat.id, msg)
    if call.data == "capricon":
        msg = random.choice(first) + ' ' + random.choice(second) + ' ' + random.choice(second_add) + ' ' + random.choice(third)
        bot.send_message(call.message.chat.id, msg)
    if call.data == "aquaries":
        msg = random.choice(first) + ' ' + random.choice(second) + ' ' + random.choice(second_add) + ' ' + random.choice(third)
        bot.send_message(call.message.chat.id, msg)
    if call.data == "pisces":
        msg = random.choice(first) + ' ' + random.choice(second) + ' ' + random.choice(second_add) + ' ' + random.choice(third)
